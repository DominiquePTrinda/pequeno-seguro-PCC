...
$imagem = $_FILES['capa_us']['name'];

//Pasta onde o arquivo vai ser salvo
$_UP['pasta'] = 'assets/IMG/CAPA/FOTOS/';
...
if ($_FILES['capa_us']['error'] != 0) {
    die("Não foi possivel fazer o upload, erro: <br />" . $_UP['erros'][$_FILES['capa_us']['error']]);
    exit; //Para a execução do script
}

//Faz a verificação da extensao do arquivo
$extensao = strtolower(end(explode('.', $_FILES['capa_us']['name'])));
...
else if ($_UP['tamanho'] < $_FILES['capa_us']['size']) {
    $status = "failed";
...
//O arquivo passou em todas as verificações, hora de tentar move-lo para a pasta foto
    ...
    if ($UP['renomeia'] == true) {
        ...
        $nome_final = time().$_FILES['capa_us']['name'];
    }
    //Verificar se é possivel mover o arquivo para a pasta escolhida
    if (move_uploaded_file($_FILES['capa_us']['tmp_name'], $_UP['pasta'] . $nome_final)) {
        //Upload efetuado com sucesso, exibe a mensagem
        $arquivo = $_UP['pasta'].'/'.$nome_final;
        $sql = "UPDATE user_pcc SET capa_user='$arquivo' WHERE id_user='$id'";
        $con->query($sql);
        $status = "Success";
